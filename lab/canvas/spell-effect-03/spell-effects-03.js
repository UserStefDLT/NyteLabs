...
class GalaxyArm_1 {
    constructor(hp = 5, size = 5, hueOn = false){
        ...
        this.dotCount = 10;
        this.angle_initial = 360/this.dotCount;
        this.angle_next = 0;
        this.len = 50;
        this.angles = [];
        this.dots = [];
    ...
    update(){
        ...
        for(let i = 0; i<this.dotCount; i++){
            if(i==0) {
                this.angle_next = this.angle_initial;
            } else {
                this.angle_next = this.angles[i-1] + 45;
            }
            this.angles[i] = this.angle_next
            let cirX = Math.cos(Math.PI*(this.hue/180)) * this.len;
            let cirY = Math.sin(Math.PI*(this.hue/180)) * this.len;
            let x = this.x + cirX;
            let y = this.y + cirY;
            // this.dots[i] = [cirX,cirY];
            this.dots[i] = [x,y];
        }
...
class GalaxyArm {
    constructor(index = 0, hp = 5, links = 5, length = 10, size = 5, angle = 45){
        this.x = s.x;
        this.y = s.y;
        this.dots = [];
        ...
        // this.hue = Math.random() * 360;
        this.hue = index * 60 + (Math.random() * 12 - 6);
        this.sat = 100;
        this.lum = 70;
        this.alp = 100;
        // this.clr = `hsla(${this.hue}, ${this.sat}%, ${this.lum}%, ${this.alp}%)`;
...
window.addEventListener('click', (ev) =>{
    ...
    if(spellBook.galaxy && click_spell.length < 200){
        for (let index = 0; index < spellBook.galaxy_arms; index++) {
            // constructor(index = 0, hp = 5, links = 5, length = 10, size = 5, angle = 45)
            click_spell.push(new GalaxyArm(index, 5, 5, 10, 2, 45));
        }
    }
    // if(spellBook.boom){
...
function moveSpark() {
    ...
    if(spellBook.rocket){
        if(mouse_particles.length < 300) {
            for (let index = 0; index < 4; index++) {
                mouse_particles.push(new Rocket(2, 1, true));
                mouse_particles.push(new Rocket(1, 1.5, true));
            }
        }
    }
    if(spellBook.huering){
        if(mouse_particles.length < 500) {
    ...
    if(spellBook.galaxy){
        if(mouse_particles.length < 1) {
            for (let index = 0; index < 1; index++) {
                // constructor(index = 0, hp = 5, links = 5, length = 10, size = 5, angle = 45)
                mouse_particles.push(new GalaxyArm(index, 3, 12, 5, 1, 60));
            }
        }
    }
}
...
/* ───── Keydown Select ───── */
var keyOptions = ['Q','W','E','R']
function keyDisplayToggle(k){
    let kUP = k.toUpperCase();
    let keyE = document.getElementById(kUP);
    keyE.classList.toggle('kbd-disable');
}
window.addEventListener("keydown", function (event) {
    ...
    switch (event.key) {
        ...
        case "r":
        case "R":
            spellBook.galaxy ? spellBook.galaxy = false : spellBook.galaxy = true;
            break;
        case " ":
